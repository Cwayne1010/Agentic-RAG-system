<script lang="ts">
	...
	function handleDrop(e: DragEvent) {
		...
		const files = Array.from(e.dataTransfer?.files ?? []).filter(
			(f) => f.type === 'text/plain' || f.type === 'text/markdown' || f.name.endsWith('.md'),
		);
...
>
	<div class="flex flex-col items-center gap-2">
		...
		<p class="text-sm font-medium">Drop .txt or .md files here</p>
		...
		<p class="text-muted-foreground text-xs">Max 10MB per file</p>
	</div>
</div>
